...
Tomei2017Single.pdf--算法文章原文
Tomei2017Singlexxx.yaml--参数配置文件
Tomei2017Single_hardware--可执行文件（设备）
Tomei2017Single_simulation可执行文件（仿真）

6. Tomei2017SingleXXX.yaml说明
freq_param: - 100.0 --干扰频率参数值
SampleTime_param: 5000.0 --采样频率参数值
Iw_param: - 1 --传递函数在扰动频率处的虚部符号值
g_param: 0.0001 --g参数值
sample_fs: 5000.0 --采样频率
dist_freq: - 100.0 --干扰频率
dist_gain:- 0.25 --干扰幅值
run_time: 10.0 --运行时间
warm_up: 5.0 --开环运行时间
start_wait: 2.0 --预热时间
...
log_folder: data/Tomei2017Single --数据存放地址
...
8. 对应文章公式在Tomei2017Single.pdf中高亮表示